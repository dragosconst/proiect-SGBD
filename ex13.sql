CREATE OR REPLACE PACKAGE proiect AS
    ...
    PROCEDURE ex6 (v_nume_carte carte.denumire%TYPE);
    PROCEDURE ex7 (vechime NUMBER,
                   cod_librarie librarie.librarie_id%TYPE);
    FUNCTION f8 (job_name jobs.denumire%TYPE)
                RETURN int_table;
    PROCEDURE p9 (lib_name librarie.denumire%TYPE);
END proiect;
/

CREATE OR REPLACE PACKAGE BODY proiect AS
    PROCEDURE ex6 (v_nume_carte carte.denumire%TYPE) IS
    ...
    PROCEDURE ex7 (vechime NUMBER,
                  cod_librarie librarie.librarie_id%TYPE)
    IS
        CURSOR c (pvec NUMBER, pcod librarie.librarie_id%TYPE) IS
            SELECT a.angajat_id, a.nume, a.prenume, a.manager_id
            FROM angajat a, lucreaza_in li
